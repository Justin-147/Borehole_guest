...
% --------------------------------------------------------------------
function Plot_Time_Data_Curve_Callback(hObject, eventdata, handles)
% 批量绘制time-data形式的二维曲线
...
if ~isempty(find(inl==2))%标注地震的情况
    ...
    %%%输入基本的参数
    %设置默认值
    dep=struct('jfw','0','wfw','500','zjxx','6.0','zjsx','10.0','ssdate','20080101','eedate','20141231','zysdxx','0','zysdsx','30');
    prompt={'距离台站的距离下限(公里)','距离台站的距离上限(公里)','震级下限','震级上限','起始日期','终止日期','震源深度下限(km)','震源深度上限(km)'};
    ...
    fields={'jfw','wfw','zjxx','zjsx','ssdate','eedate','zysdxx','zysdsx'};
...
for iiNFZ=1:1:NFZ
    ...
    if ~isempty(find(inl==2))%
        if  ~isempty(tkkx)
            ...
            [txfznum,txzhenji]=TiaoXuanDiZhen1(1234,1,dzfile,dzoutname,jingdu,weidu,...
