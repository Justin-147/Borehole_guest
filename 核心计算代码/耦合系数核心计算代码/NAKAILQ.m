function NAKAILQ(handles,wb,lx)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%显示提示信息
if wb==1
...
tic;%开始计时
if wb==1
    hw=waitbar(0,'Nakai拟合计算中...','Name',[handles.pny handles.fny]);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%参数赋值
%处理的参数选择
...
if ds>0
    %计算理论固体潮汐值
    if lx==1
        [datazV,tmp]=TheoryTide(FB,FL,HH,IHS,timet,AZ);%线应变
    elseif lx==2
...
elseif MX==4
    for ii=1:1:ds
        ...
        dataP=datazP(1+db*(ii-1):dc+db*(ii-1));
        dataW=datazW(1+db*(ii-1):dc+db*(ii-1));
        RK=dxnhN(MX,dataY,dataV,dataP,dataW);
        RKK=[RKK,RK];
        timej=[timej;timeu(WZ)];
        if wb==1
            waitbar(ii/ds,hw);
...
fprintf(fido,'%8i %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f\n',[timej';RKK]);
fclose(fido);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ttu=toc/60;
if wb==1
    tinf={['计算完毕======>','耗时',num2str(ttu),'分钟'];...
        ['耦合系数的计算结果保存在',outname,'中'];...
        '                                                   ';...
        '每一列依次为：';...
        '耦合系数、耦合滞后（小时）、气压系数、气压滞后（小时）、';...
        '水位系数、水位滞后（小时）、常数项、一次项系数、二次项系数'};
